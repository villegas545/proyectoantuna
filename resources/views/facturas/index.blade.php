...
<html lang="en">
    ...
    <body id="page-top">
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top" id="sideNav">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link js-scroll-trigger" href="#fa">Facturas</a></li>
        ...
        <div class="container-fluid p-0">
            <!-- About-->
            <section class="resume-section" id="fa">
                <div class="resume-section-content">
                    <h3 class="mb-0"> Facturas</h3><td >Inicio (despliegue de datos)<a href="#agregar">Agregar</a> |
  ...
  <thead style="background-color: #fff">
    <tr style="color: black ">
      ...
      <th scope="col">Número de liquidacion</th>
...
<tbody style="background-color: #fff">
    ...
    <tr style="color: black ">
      ...
      <td>{{$facturas->no_liquidacion}}</td>
      <td>{{$facturas->observaciones}}</td>
      <td>{{$facturas->estatus}}</td>
      <td>{{$facturas->total}}</td>
      <td>{{$facturas->debe}}</td>
      <td>
        {{-- <a href="{{ url('/facturas/'.$facturas->id.'/edit') }}">
          Editar
        </a> --}}
      <button class="btn btn-succes modificarboton" data-toggle="modal"  data-target="#ventanaModal"  valor="{{$facturas->id}}">Editar</button> 
 ...
 <a class="nav-link js-scroll-trigger" href="../abono/{{ $abono->NoFactura }}">Abonos</a>
       |

        <form method="post" action="{{ url('/facturas/'.$facturas->id) }}">
        {{csrf_field()  }}
        {{ method_field('DELETE') }}
...
<center>
    <form action="{{ url('/facturas')}}" method="POST">
      {{ csrf_field() }}
      <label style="color: black">{{'Numero de factura'}}</label>
      <input type="text" style="width:30%;"class="form-control" name="no_factura" placeholder="numero de factura">
      
      <label style="color: black">{{'Numero de liquidacion'}}</label>
      ...
      <input type="text" style="width:30%;"class="form-control" name="observaciones" placeholder="observaciones">
      
      <label style="color: black">{{'Estatus'}}</label>
      <input type="text" style="width:30%;"class="form-control" name="estatus" placeholder="estatus">
      
      <label style="color: black">{{'Total'}}</label>
      <input type="text" style="width:30%;"class="form-control" name="total" placeholder="total">
      
      <label style="color: black">{{'Debe'}}</label>
      <input type="text" style="width:30%;"class="form-control" name="debe" placeholder="cantidad que debe">
        ...
        <script type="text/javascript">
          $(document).ready(function(){
              $(".modificarboton").click(function(){
                ...
                var url="{{ url('/facturas/') }}valor";
                 $.ajax({
                url: "funcionesjquery.php",
                type: "POST",
                dataType: "json",
                data: {jselector:'modificarfactura',jvalor:valor,jtoken:token,jurl:url}
